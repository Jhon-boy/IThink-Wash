...
class UsuariosRepositoryImpl implements UsuariosRepository {
  ...
  @override
  Future<Either<Failure, TsegUsuarioEntity>> getUsuarioById(int idUsuario) async {
    final isConnected = await _connectivity.checkConnection();
    if (!isConnected) {
      return const Left(NetworkFailure('No hay conexión a internet'));
    }
  ...
  Future<Either<Failure, TsegUsuarioEntity>> createUsuario(
      TsegUsuarioEntity usuario) async {
    final isConnected = await _connectivity.checkConnection();
    if (!isConnected) {
      return const Left(NetworkFailure('No hay conexión a internet'));
    }
  ...
  @override
  Future<Either<Failure, TsegUsuarioEntity>> toggleEstadoUsuario(int idUsuario,
