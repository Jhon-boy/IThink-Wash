...
class OrdenCardWidget extends ConsumerStatefulWidget {
  ...
  const OrdenCardWidget({super.key, required this.orden, required this.onTap});
...
class _OrdenCardWidgetState extends ConsumerState<OrdenCardWidget> {
  ...
  Future<void> _cargarDetalle() async {
    try {
      final repo = OrdenRemoteRepository(OrdenRemoteDataSource(ref: ref));
      final r = await repo.getDetallesByOrdenEntity(widget.orden.idOrden!);
      r.fold((l) => null, (detalles) {
        if (detalles.isNotEmpty && mounted) {
          setState(() =>
              _primeraDescripcion = detalles.first.descripcionPrenda ?? '');
        }
      });
    } catch (_) {}
  }
            ...
            children: [
              ...
              Expanded(
                  child: Column(
                    ...
                    Row(children: [
                      Expanded(
                          child: Text(o.numeroOrden ?? 'Orden #${o.idOrden}',
                    ...
                    Row(children: [
                      ...
                      Flexible(
                          child: Text(_cargando ? '...' : _nombreCliente,
                              style: const TextStyle(
                                  fontSize: 13,
                    ...
                    Row(children: [
                      if (_primeraDescripcion.isNotEmpty) ...[
                        const SizedBox(width: 10),
                        Flexible(
                            child: Text(_primeraDescripcion,
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: const TextStyle(
                                    fontSize: 12,
                        ...
                        children: [
                          Row(children: [
                            ...
                            if (o.saldoPendiente != null &&
                              ...
                              Text(
                                  'Saldo: \$${o.saldoPendiente!.toStringAsFixed(2)}',
                                  style: TextStyle(
                                      fontSize: 13,
                                      fontWeight: FontWeight.w600,
